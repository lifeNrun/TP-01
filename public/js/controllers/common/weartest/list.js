...
function ($scope, $location, $http, WTServices, async, notificationWindow) {
    ...
    var adjustShownWeartests = function () {
        ...
        if ($scope.shownWeartests.length >= allWeartestsLength) {
        ...
        for (i = 0; i < allWeartestsLength; i++) {
            ...
            for (j = 0; j < $scope.shownWeartests.length; j++) {
        ...
        $scope.shownWeartests.push.apply($scope.shownWeartests, notShownWeartests);
